			...
			for (int i = 0; i < methods.size(); ++i) {
				...
				if (method.name.equals("testNull"))
					if (method.instructions.size() > 0) {
						...
						Topologic<CFG, Edge, BasicBlock> topologic = new Topologic<CFG, Edge, BasicBlock>(cfg);
						topologic.accept(new visitors());
						System.out.println("RevertTopologic result:" + method.name);
						ReverseTopologic<CFG, Edge, BasicBlock> reverTo = new ReverseTopologic<CFG, Edge, BasicBlock>(
								cfg);
						reverTo.accept(new visitors());
						System.out.println();
					}
			}
		} catch (IOException e) {
