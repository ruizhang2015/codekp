...
// 跨过程的指向分析算法，该过程输出结果更为全面，包括：别名信息，type信息，引用类型变量的object信息[context-sensitive]，基本类型（值类型）变量的value信息，isRead，isWriten，lastAssignJIRValues
...
//						Integer ie = exceptionalblock.getExceptionalObjectCreationStmt();
//
//						if (ie != null) {
//							// roll back the object creation effect on the
//							// points-to
//							// info
//							Stmt objCreationStmt = exceptionalblock.stmts.get(ie);
//							if (objCreationStmt instanceof AssignStmt) {
//								AssignStmt assignStmt = (AssignStmt) objCreationStmt;
//
//								// to judege whether the points-to analysis
//								// result
//								// contains the assigned variable, then cancel
//								// the
//								// effect.
//								this.deTransferStmt(assignStmt, fact);
//							}
//						}
//					}
//				}
//			}
			...
			try {
				...
				e.printStackTrace();
				// System.out.println("this block start stmt:" + block.startStmt
				// + " end stmt:" + block.endStmt);
			}
	...
	public void transferStmt(Stmt stmt, HashMap<InterJIRValue, InterVarInfo> fact) {
		if (fact == null)
			fact = new HashMap<InterJIRValue, InterVarInfo>();
	...
	public HashMap<InterJIRValue, InterVarInfo> transferAssignmentStmt(HashMap<InterJIRValue, InterVarInfo> fact,
			JIRValue leftValue, int leftValueLevel, JIRValue rightValue, int rightValueLevel) {
		// a = x
		// a.f = x
		// a = x.f
			...
			if (leftValue instanceof LocalRef || leftValue instanceof TempRef) {
				...
				if (rightValue instanceof AnyNewExpr) {
					...
					if (rightValue instanceof NewExpr) {
						...
						String objectName = "O[" + newExpr.getType().toString() + "][" + currentLine + "]" + "["
								+ this.level + "]" + "[" + newExpr.sizes.length + "]";
					...
					if (rightValue instanceof CaughtExceptionRef) {
						...
						objectName = "O[" + ref.getType().toString() + "][" + currentLine + "]" + "[" + this.level
								+ "]";
						InterVarInfo newPointsToInfo = new InterVarInfo();
						newPointsToInfo.types.add(objectType);
						newPointsToInfo.objects.add(objectName);
						...
						// 删除a的所有关联信息
						InterVarInfo leftPointsToInfo = new InterVarInfo();
						InterVarInfo rightNodePointsToInfo = fact.get(new InterJIRValue(rightValue, rightValueLevel));

						// 获取a的别名信息，将a从与其别名的ref的alias删除
						if (fact.get(new InterJIRValue(leftValue, leftValueLevel)) != null) {
							for (InterJIRValue alias : fact.get(new InterJIRValue(leftValue, leftValueLevel)).alias) {
								if (fact.get(alias) != null && fact.get(alias).alias != null) {
									fact.get(alias).alias.remove(new InterJIRValue(leftValue, leftValueLevel));
								}
							}
						}
				...
				if (rightValue instanceof AnyNewExpr) {
					...
					objectName = "O[" + objectType.toString() + "][" + currentLine + "]" + "[" + this.level + "]" + ":"
					...
					// 获取a别名信息，并相应增加a.b的别名信息
					InterVarInfo baseInterPointsToInfo = fact.get(new InterJIRValue(leftFieldRef.base, leftValueLevel));
					if (baseInterPointsToInfo != null) {
						for (InterJIRValue alias : baseInterPointsToInfo.alias) {
							FieldRef newfieldRef = (FieldRef) leftFieldRef.clone();
							newfieldRef.base = alias.intraJIRValue;
	...
	private void transferRefAssignment(HashMap<InterJIRValue, InterVarInfo> fact, JIRValue leftValue,
		...
		InterJIRValue interValue = new InterJIRValue(rightValue, rightValueLevel);
		newPointsToInfo.lastAssignJIRValues.add(interValue);
		fact.put(new InterJIRValue(leftValue, leftValueLevel), newPointsToInfo);
			...
			if (me != null) {
				...
				if (invoke.isStatic() || !me.isAbstract()) {// 递归分析
					...
					try {
						...
						HashMap<InterJIRValue, InterVarInfo> calleeResultFact = calleeAnlysis.getResultFact(calleeCFG
								.getExit());
	...
	public Iterator<Edge> logicalSuccessorEdgeIterator(BasicBlock block) {
	...
	public synchronized void execute() throws AnalyzerException {
		...
		do {
			...
			while (i.hasNext()) {
				...
				// "-" + this.cfg.getMethod().getFullName() + ":" +
				// block.toString());
//				System.out.println(block.toString());
//				System.out.println(block.startStmt);
...
//				}
				...
				if (!isEndBlock && start != null) {
					// if the original start is
					...
					while (predEdgeIter.hasNext()) {
						...
						if (edge.getType() == EdgeType.HANDLED_EXCEPTION_EDGE) {
							Integer ie = logicalPred.getExceptionalObjectCreationStmt();

							if (ie != null) {
								// roll back the object creation effect on
								// the points-to
								// info
								Stmt objCreationStmt = logicalPred.stmts.get(ie);
								if (objCreationStmt instanceof AssignStmt) {
									AssignStmt assignStmt = (AssignStmt) objCreationStmt;

									// to judege whether the points-to
									// analysis result
									// contains the assigned variable, then
									// cancel the
									// effect.
									this.deTransferStmt(assignStmt, pred);
								}
							}
						}
						...
						// rolling back the exceptional events
						// List<BasicBlock> exceptionalblocks =
						// block.getIncomingExceptionBlocks();
						// if (exceptionalblocks != null &&
						// exceptionalblocks.size() > 0) {
						//
						// // to identify the effects of what blocks needs to
						// // be rolled
						// // back
						// for (BasicBlock exceptionalblock : exceptionalblocks)
						// {
						// Integer ie =
						// exceptionalblock.getExceptionalObjectCreationStmt();
						//
						// if (ie != null) {
						// // roll back the object creation effect on
						// // the points-to
						// // info
						// Stmt objCreationStmt =
						// exceptionalblock.stmts.get(ie);
						// if (objCreationStmt instanceof AssignStmt) {
						// AssignStmt assignStmt = (AssignStmt) objCreationStmt;
						//
						// // to judege whether the points-to
						...
						// // cancel the
						// // effect.
						// this.deTransferStmt(assignStmt, newStart);
						// }
						// }
						// }
						//
	...
	public static void main(String[] args) {
		...
		for (MethodNode method : cc.methods) {
			...
			if (method
					.getOriginalFullName()
					.equals(
							"org.jfree.chart.servlet.ServletUtilities::sendTempFile(Ljava/io/File;Ljavax/servlet/http/HttpServletResponse;Ljava/lang/String;)")) {
				continue;
			}
	...
	public void deTransferStmtWhenFixPatternMatching(AssignStmt assignStmt,
			HashMap<InterJIRValue, InterVarInfo> currentVarInfoWhenMatching) {
		InterJIRValue jirValue = new InterJIRValue(assignStmt.left, 0);
		InterVarInfo varInfo = currentVarInfoWhenMatching.get(jirValue);
		if (varInfo != null && varInfo.objects != null && varInfo.objects.size() > 0) {
			Set<String> newObjs = new HashSet<String>();
			for (String object : varInfo.objects) {
				if (!object.endsWith("[fail-creation]")) {
					newObjs.add(object);
					newObjs.add(object + "[fail-creation]");
				} else
					newObjs.add(object);
			}
	...
	public void deTransferStmt(AssignStmt assignStmt, HashMap<InterJIRValue, InterVarInfo> currentVarInfoWhenMatching) {
		InterJIRValue jirValue = new InterJIRValue(assignStmt.left, 0);
		InterVarInfo varInfo = currentVarInfoWhenMatching.get(jirValue);
		if (varInfo != null && varInfo.objects != null && varInfo.objects.size() > 0) {
			Set<String> newObjs = new HashSet<String>();
