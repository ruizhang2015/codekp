...
import automachine.VisitFieldInsnEdge;
import automachine.VisitIincInsnEdge;
import automachine.VisitInsnEdge;
import automachine.VisitIntInsnEdge;
import automachine.VisitJumpInsnEdge;
import automachine.VisitLocalVariableEdge;
import automachine.VisitLookupSwitchInsnEdge;
import automachine.VisitMethodInsnEdge;
import automachine.VisitMultiANewArrayInsnEdge;
import automachine.VisitTableSwitchInsnEdge;
import automachine.VisitTryCatchBlockEdge;
import automachine.VisitTypeInsnEdge;
import automachine.VisitVarInsnEdge;
 ...
 * @author root
 */
public class AutoMachineMethodAdapter extends MethodDetector {
	...
	public AutoMachineMethodAdapter() {
		super();
	}

	public AutoMachineMethodAdapter(String specificFile) {
	...
	public AutoMachineMethodAdapter(int automachineId) {
	...
	public AutoMachineMethodAdapter(AutoMachine am) {
	...
	public void visitCode() {
		...
		// this.cfg.getMethod());
		this.instructionFrame = node.getCFG().getFrames();
		// this.instructionFrame=new Analyzer(new
		// BasicInterpreter()).analyze(node.owner, node);
	}
	...
	private void reportAmBug(AutoMachineException e, int line) {
		reportor.report(this.getClassName(), new BugInstance(e.getMessage() + " by am:" + this.automachineId, line));
	}

	private void reportAmBug(String bugMsg, int line) {
		reportor.report(this.getClassName(), new BugInstance(bugMsg + " by am:" + this.automachineId, line));
	}
